...
public class IncomeGroupAuthorizationHandler(IHttpContextAccessor httpContextAccessor, DatabaseContext dbContext)
        ...
        IncomeAuthorizationRequirement requirement)
    {
        ...
        if (userIdClaim != null)
        {
            ...
            if (int.TryParse(userIdClaim.Value, out var userId))
            {
                ...
                var incomeGroup = await _dbContext.IncomeGroups.FirstOrDefaultAsync(b => b.Id == incomeGroupId);

                if (incomeGroup != null && incomeGroup.UserId == userId)
                {
