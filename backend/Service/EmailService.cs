...
public class EmailService(IConfiguration configuration) : IEmailService
{
    public async Task<bool> SendEmail(EmailRequestDto emailRequest, string subject, string body)
    {
        try
        {
            var smtpClient = new SmtpClient(configuration["Mail:Client"] ?? "smtp.gmail.com")
            {
                Port = 587,
                Credentials =
                    new NetworkCredential("perisicnikola37@gmail.com",
                        configuration["Mail:Secret"] ?? "cqzy dvds ngoo pdqm"),
                EnableSsl = true
            };
    ...
    public async Task<bool> SendEmailWithAttachment(EmailRequestDto emailRequest, string subject, string body,
        string attachmentFileName, byte[] attachmentContent, string attachmentContentType)
    {
        try
        {
            ...
            {
                Port = 587,
                Credentials = new NetworkCredential("perisicnikola37@gmail.com",
                    configuration["Mail:Secret"] ?? "cqzy dvds ngoo pdqm"),
                EnableSsl = true
            };
