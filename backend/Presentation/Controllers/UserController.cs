...
public class UserController(DatabaseContext context, IEmailService emailService, IValidator<User> validator, PdfGenerator pdfGenerator)
	: ControllerBase
{
	// GET: api/User
	[HttpGet]
	...
	public async Task<IActionResult> SendEmail([FromBody] EmailRequestDto emailRequest)
	{
		try
		{
			var user = await context.Users
				.Include(u => u.Incomes)
				.FirstOrDefaultAsync(u => u.Id == 1); // Assuming emailRequest.To contains the user's email
        ...
        <html dir=""ltr"" lang=""en"">
          ...
          <body style=""background-color:#ffffff;font-family:'Inter', 'ui-sans-serif', 'system-ui', '-apple-system', 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans', sans-serif"">
			...
			await emailService.SendEmailWithAttachment(emailRequest, "subject", emailBody, "filename.pdf", pdfBytes, "application/pdf");
